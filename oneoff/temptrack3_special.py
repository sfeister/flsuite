...
# -*- coding: utf-8 -*-
"""
temptrack2.py: Track the temperature, Brms, etc. evolution through time in two simulations. Special use case.
...
USAGE:
mpirun -np 12 python temptrack3_special.py
...
def customline1(anlsT, fld, fld_mult, name, units, tgv, outdir, simname, edges_um, tmin=15, tmax=30, logit=False):
    ...
    ax = fig.add_subplot(111)
    for edge_um in edges_um:
        yvals = anlsT['centcube_mean_' + str(int(edge_um)) + 'um_' + fld] * fld_mult # Remember to multiply by the multiplier
        ax.plot(tgv, yvals, label="l=" + str(int(edge_um)) + " um")
    ax.set_xlim([tmin, tmax])
    ax.set_xlabel("Simulation time (ns)")
    ax.set_ylabel("Mean of volume: " + ylabel)
    ax.set_ylim(ymin=0)
    ax.set_title(name + ', Cube (edge len. l) @ TCC')
    ax.minorticks_on()
    ...
    fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
    plt.tight_layout()
    fig.savefig(os.path.join(outdir, 'Central cube (Mean) - ' + name + '.png'), dpi=150)

def plotTcentcube2(anlsT, outdir='.'):
    """ A more specific plot the summarized results from anlzDcentcube """
    print("Making SPECIFIC central cube plots...")
    simname = os.path.basename(os.path.normpath(outdir)) # Use the name of the output directory as a label for this sim., to use in plots etc.
    ...
    name = "Mass density"
    units = "g/cm^3"
    fig = customline1(anlsT, fld, fld_mult, name, units, tgv, outdir, simname, edges_um)
...
def anlzD(ds, outdir='.'):
    ...
    anlsD = anlzDcentcube(ds, anlsD, [100, 250, 500, 1000], ['tele', 'nele', 'dens'])
...
def plotT(anlsT, outdir='.'):
    ...
    plotTcentcube2(anlsT, outdir=outdir)
...
if __name__ == "__main__":
    ...
    fnpatt = os.path.join(datdir, basenm + 'hdf5_plt_cnt_?[1,2]??') # yt-time-series filename pattern for plot files
    # hdf5_plt_cnt_????
    # hdf5_plt_cnt_???[0,2,4,6,8]
    # hdf5_plt_cnt_???[0,5]
    # hdf5_plt_cnt_??[0,2,4,6,8]0
    # hdf5_plt_cnt_??[0,5]0
    outroot=r'/gpfs/mira-home/sfeister/myouts/2017-07-21 Quick temp and dens'
