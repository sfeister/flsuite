...
def plotTsph(anlsT, outdir='.'):
    ...
    fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
    plt.tight_layout()
    fig.savefig(os.path.join(outdir, 'Energy vs time - Magnetic (lin).png'))

    ax.set_ylim(np.array(ax.get_ylim())/20.0)
    fig.savefig(os.path.join(outdir, 'Energy vs time - Magnetic (lin2).png'))
    
    ax.set_yscale('log')
    ax.set_ylim(1e-7, 1e3)
    ax.legend(loc='lower right')
    plt.tight_layout()
    fig.savefig(os.path.join(outdir, 'Energy vs time - Magnetic (log).png'))
    ...
    fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
    plt.tight_layout()
    fig.savefig(os.path.join(outdir, 'Energy vs time - Kinetic (lin).png'))

    ax.set_ylim(np.array(ax.get_ylim())/20.0)
    fig.savefig(os.path.join(outdir, 'Energy vs time - Kinetic (lin2).png'))
    
    ax.set_yscale('log')
    ax.set_ylim(1e-7, 1e3)
    ax.legend(loc='lower right')
    plt.tight_layout()
    fig.savefig(os.path.join(outdir, 'Energy vs time - Kinetic (log).png'))
...
def plotTstreak(anlsT, outdir='.'):
    ...
    ## LINE Plots
    for lbl in ["line1", "line_500um", "line_2mm"]: # Iterate over the lineouts (defined above), make plots for each
        ...
        ax.set_title("Velocity Z-component: Lineout X=Y=0, through time")
        ...
        fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
        fig.savefig(os.path.join(outdir, lbl + "Velz_streak.png"), dpi=300)
        ...
        ax.set_title("Magnetic field magnitude: Lineout X=Y=0, through time")
        lstag = ax.plot(tgv, zstag, lw=1, c='black', linestyle='--', label='Stagnation') # Add the stagnation z position (in mm) to plot
        ax.legend(handles=[lstag[0]], loc='upper right')
        fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
        fig.savefig(os.path.join(outdir, lbl + "Btot_streak.png"), dpi=300)    
        ...
        ax.set_title("Magnetic energy density: Lineout X=Y=0, through time")
        lstag = ax.plot(tgv, zstag, lw=1, c='black', linestyle='--', label='Stagnation') # Add the stagnation z position (in mm) to plot
        ax.legend(handles=[lstag[0]], loc='upper right')
        fig.text(0.99, 0.01, simname, horizontalalignment='right') # Lower-right footer
        fig.savefig(os.path.join(outdir, lbl + "MagEnergy_streak.png"), dpi=300)
    plt.close('all')
...
if __name__ == "__main__":
    ...
    datdir = r'/projects/CosmicLaser/tzeferac/SCRIPT1/RUN2'
    basenm = r'tdyno2016_' # Prefix for plot filenames, which is defined as "basenm" in the flash.par file

    simname = "OMEGA_NLUF6"
    datdir = r'/projects/Omega-NIF_Exp/tzeferac/NLUF6grpFINAL/SCRIPT5/RUN1'
    ...
    #outroot = r'/home/sfeister/myouts/TDYNO_BAND' # Folder to store outputs
